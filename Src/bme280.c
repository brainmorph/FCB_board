...
uint8_t bme280ReadReg(uint8_t reg)
{
	...
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c3,
			shiftedAddress, pData, 1, 1000); //select register
	if(status != HAL_OK)
	{
		// TODO: log error
		log_incrementErrorCount();
	...
	status = HAL_I2C_Master_Receive(&hi2c3,
			shiftedAddress, &value, 1, 1000); //read from register
	return value;
}
...
void bme280ReadRegs(uint8_t reg, uint16_t size, uint8_t* data)
{
	...
	status = HAL_I2C_Master_Receive(&hi2c3,
			shiftedAddress, data, size, 1000); //read size bytes from register
...
void bme280WriteReg(uint8_t reg, uint8_t value)
{
	...
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c3,
			shiftedAddress, pData, Size, 1000); //select register and write to it all in one
	if(status != HAL_OK)
	{
		// TODO: log error
		log_incrementErrorCount();
